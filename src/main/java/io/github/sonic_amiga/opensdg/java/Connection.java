// Vendored from opensdg_java 1.0.0 to shadow the dependency class.
// Single change vs upstream: doSendRawData() cancels the pending socket write on
// TimeoutException so it does not dangle and trigger WritePendingException on the
// next write. Keep in sync with upstream on dependency bumps.
// Updated: 2026-06-15 13:20
...
public abstract class Connection extends IConnection {
    ...
    protected void doSendRawData(ByteBuffer data)
        ...
        synchronized (writeLock) {
            while (size > 0) {
                Future<Integer> write = s.write(data);
                int ret;
                try {
                    ret = write.get(timeout, TimeUnit.SECONDS);
                } catch (TimeoutException e) {
                    // Cancel the still-pending write; otherwise it keeps the channel in a
                    // write-pending state and the next write throws WritePendingException.
                    write.cancel(true);
                    throw e;
                }
                size -= ret;
